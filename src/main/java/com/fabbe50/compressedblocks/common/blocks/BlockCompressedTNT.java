 ...
 */
public class BlockCompressedTNT extends BlockBase {
    public static final PropertyBool EXPLODE = PropertyBool.create("explode");

    public BlockCompressedTNT(Material material, MapColor mapColor, String name, float hardness, float resistance, CreativeTabs tab) {
        ...
        if (!worldIn.isRemote) {
            if (Configs.compressedTNTSpread) {
                for (int i = 0; i < CompressedTNTPositionalData.getCompressed(getCompression()).length; i++) {
                    for (int j = 0; j < CompressedTNTPositionalData.getCompressed(getCompression()).length; j++) {
                        EntityCompressedTNT entityTNT = new EntityCompressedTNT(worldIn, (double) ((float) pos.getX() + 0.5F + CompressedTNTPositionalData.getCompressed(getCompression())[i]), (double) pos.getY(), (double) ((float) pos.getZ() + 0.5F + CompressedTNTPositionalData.getCompressed(getCompression())[j]), explosionIn.getExplosivePlacedBy(), getCompression());
                        entityTNT.setFuse(2);
                ...
                for (int j = 0; j < getTNTStack(); j++) {
                    EntityCompressedTNT entityTNT = new EntityCompressedTNT(worldIn, (double) ((float) pos.getX() + 0.5F), (double) pos.getY(), (double) ((float) pos.getZ() + 0.5F), explosionIn.getExplosivePlacedBy(), getCompression());
                    entityTNT.setFuse(2);
                    worldIn.spawnEntity(entityTNT);
                }
            }
        ...
        if (!worldIn.isRemote) {
            if ((state.getValue(EXPLODE))) {
                if (Configs.compressedTNTSpread) {
                    for (int i = 0; i < CompressedTNTPositionalData.getCompressed(getCompression()).length; i++) {
                        for (int j = 0; j < CompressedTNTPositionalData.getCompressed(getCompression()).length; j++) {
                            EntityCompressedTNT entityTNT = new EntityCompressedTNT(worldIn, (double) ((float) pos.getX() + 0.5F + CompressedTNTPositionalData.getCompressed(getCompression())[i]), (double) pos.getY(), (double) ((float) pos.getZ() + 0.5F + CompressedTNTPositionalData.getCompressed(getCompression())[j]), igniter, getCompression());
                            entityTNT.setFuse(2);
                            worldIn.spawnEntity(entityTNT);
                            worldIn.playSound((EntityPlayer) null, entityTNT.posX, entityTNT.posY, entityTNT.posZ, SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.BLOCKS, 1.0F, 1.0F);
                        }
                    }
                }
                else {
                    for (int j = 0; j < getTNTStack(); j++) {
                        EntityCompressedTNT entityTNT = new EntityCompressedTNT(worldIn, (double) ((float) pos.getX() + 0.5F), (double) pos.getY(), (double) ((float) pos.getZ() + 0.5F), igniter, getCompression());
                        entityTNT.setFuse(2);
                        worldIn.spawnEntity(entityTNT);
                    }
                }
