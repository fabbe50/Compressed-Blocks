...
public class BlockFurnaceCompr extends BlockContainer {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (worldIn.isRemote) {
            ...
            if (tileentity instanceof TileEntityFurnaceCompr) {
                playerIn.displayGUIChest((TileEntityFurnaceCompr)tileentity);
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        ...
        if (stack.hasDisplayName()) {
            TileEntity tileentity = worldIn.getTileEntity(pos);
            if (tileentity instanceof TileEntityFurnaceCompr) {
                ((TileEntityFurnaceCompr)tileentity).setCustomInventoryName(stack.getDisplayName());
            }
        }
    }
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        if (!keepInventory) {
            TileEntity tileentity = worldIn.getTileEntity(pos);
            if (tileentity instanceof TileEntityFurnaceCompr) {
                InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntityFurnaceCompr)tileentity);
    ...
    public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("Smelts items " + TileEntityFurnaceCompr.getSpeedMultiplier() + " times faster.");
    }
}