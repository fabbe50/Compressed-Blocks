 ...
 */
public class TileEntityBrewer extends TileEntityBrewingStand {
    ...
    public void update() {
        ...
        if (!this.world.isRemote) {
            ...
            if (!Arrays.equals(aboolean, this.filledSlots)) {
                ...
                if (!(iblockstate.getBlock() instanceof BlockBrewer)) {
                    return;
                }

                for (int i = 0; i < BlockBrewer.HAS_BOTTLE.length; ++i) {
                    iblockstate = iblockstate.withProperty(BlockBrewer.HAS_BOTTLE[i], Boolean.valueOf(aboolean[i]));
                }
    ...
    public static void registerFixesBrewingStand(DataFixer fixer) {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntityBrewer.class, new String[] {"Items"}));
    }
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        if (index == 3) {
            ...
            return index == 4 ? item == Items.BLAZE_POWDER : VanillaHooks.isValidInput(stack);
        }
    }
    ...
    public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn) {
        return new ContainerBrewer(playerInventory, this);
    }
