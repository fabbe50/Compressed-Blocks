...
public class BeaconCraftingRecipeChecker {
    private BeaconCraftingRecipeChecker() {
    }

    public static List<IRecipe> getValidRecipes(final IJeiHelpers jeiHelpers) {
        CraftingRecipeValidator<MiniBeaconRecipes> endgameRecipeValidator = new CraftingRecipeValidator<MiniBeaconRecipes>() {
            @Override
            protected IRecipeWrapper getRecipeWrapper(MiniBeaconRecipes recipe) {
                return new BeaconCraftingRecipeWrapper(jeiHelpers, recipe);
            }

            @Override
            protected int getInputCount(MiniBeaconRecipes recipe) {
                return getInputCount(recipe.getInput());
            }
        };

        List<IRecipe> recipes = new ArrayList<>(ModRegistry.beaconRecipes);
        List<IRecipe> validRecipes = new ArrayList<>(recipes.size());
        Iterator var1 = recipes.iterator();

        while(var1.hasNext()) {
            IRecipe recipe = (IRecipe)var1.next();
            if (recipe instanceof MiniBeaconRecipes) {
                if (endgameRecipeValidator.isRecipeValid((MiniBeaconRecipes) recipe)) {
